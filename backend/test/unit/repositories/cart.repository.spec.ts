...
describe('CartRepository', () => {
  ...
  describe('findByUserAndStore', () => {
    it('should find one cart by user and store with relations', async () => {
      (repository.findOne as jest.Mock).mockResolvedValue(mockCart);

      const result = await repository.findByUserAndStore('user-1', 'store-1');

      expect(repository.findOne).toHaveBeenCalledWith({
        where: { user: { id: 'user-1' }, store: { id: 'store-1' } },
  ...
  describe('findWithItems', () => {
    it('should find one cart with items, variant, store, and user relations', async () => {
      (repository.findOne as jest.Mock).mockResolvedValue(mockCart);
