...
export const UPLOADS_NEWS_DIR = './uploads/news';
export const ALLOWED_IMAGE_MIME_PREFIX = 'image/';

@Injectable()
export class NewsFileService {
  private readonly logger = new Logger(NewsFileService.name);
  ...
  async saveFile(
    ...
    const finalDir = join(UPLOADS_NEWS_DIR, newsId, type);
    ...
    const relativePath = `/uploads/news/${newsId}/${type}/${filename}`;
